...
def sort_by_formation_count(acces, asc):
    ...
    return acces.annotate(
        Count('etablissement__formation')).order_by(
        prefix + 'etablissement__formation__count')
...
class ListeEtablissementsViewModel(object):
    ...
    columns = map(
        ...
        [
            ('region', make_sort_by_column('etablissement__region'), 'Région'),
    ...
    sort_name2sort_function = dict(
        (col.sort_name, col.sort_function) for col in columns)
    ...
    def _sort(self, sort):
        if sort and sort.strip('-') in self.sort_name2sort_function:
            asc = sort[0] != '-'
            ...
            self.liste_acces = self.sort_name2sort_function[sort_name](
                self.liste_acces, asc)
        else:
            self.liste_acces.order_by(
