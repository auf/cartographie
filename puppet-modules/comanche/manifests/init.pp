...
#    purge       => true,
  ...
  case "${lsbdistid}" {
    /(CentOS|RedHatEnterpriseServer)/ : {
      if (!defined(Comanche::Module['logio']))      { comanche::module { 'logio': } }
      if (!defined(Comanche::Module['log_config'])) { comanche::module { 'log_config': } }
    }
  }

#  comanche::module { 'authz_host': }
#  comanche::module { 'log_config': }
#  comanche::module { 'logio': }
#  comanche::module { 'alias': }
#  comanche::module { 'dir': conffile => true}
#  comanche::module { 'mime': conffile => true }
  ...
  # not working with centos, to fix...
  @package{'libapache2-mod-jk':}
  comanche::module { $apache2_modules:
    apache2_rootdir => $apache2_rootdir
  }
