class backup::rsnapshot::client (
  ...
  $rsnapshot_logfile         = "/var/log/rsnapshot-${client}-${fqdn}.log",
  $rsnapshot_lockfile        = "/var/run/rsnapshot-${client}-${fqdn}.lock",
  ...
  if $rsnapshot_hourly_retain {
    @@cron { "cron_backup_${server}_${client}_hourly}":
      command  => "/usr/local/sysadmin/rsnapshot -v -c $rsnapshot_conf_dir/$client/$server hourly >> /var/backups/log/$client/$server/hourly.log",
      user     => root,
  ...
  if $rsnapshot_daily_retain {
    @@cron { "cron_backup_${server}_${client}_daily":
      command  => "/usr/local/sysadmin/rsnapshot -v -c $rsnapshot_conf_dir/$client/$server daily >> /var/backups/log/$client/$server/daily.log",
      user     => root,
  ...
  if $rsnapshot_weekly_retain {
    @@cron { "cron_backup_${server}_${client}_weekly":
      command  => "/usr/local/sysadmin/rsnapshot -v -c $rsnapshot_conf_dir/$client/$server weekly >> /var/backups/log/$client/$server/weekly.log",
      user     => root,
  ...
  if $rsnapshot_monthly_retain {
    @@cron { "cron_backup_${server}_${client}_monthly":
      command  => "/usr/local/sysadmin/rsnapshot -v -c $rsnapshot_conf_dir/$client/$server monthly >> /var/backups/log/$client/$server/monthly.log",
      user     => root,
