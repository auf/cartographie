...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for formation in orm['formation.Formation'].objects.all():
            for partenaire_autre in formation.partenaires_autres.all():
                old_pk = partenaire_autre.pk
                partenaire_autre.pk = None
                partenaire_autre.etablissement = formation.etablissement
                partenaire_autre.save()
                f = orm['formation.FormationPartenaireAutre']()
                f.formation = formation
                f.etablissement = partenaire_autre
                f.save()
                orm['formation.EtablissementAutre'].objects.get(pk=old_pk).delete()
            formation.save()
