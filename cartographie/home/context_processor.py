...
def stats(request):
    def _formations():
        return str(
            Formation.objects.exclude(
                statut=StatutsFormation.supprimee).count())

    def _etablissements():
        data = Formation.objects.exclude(
            statut=StatutsFormation.supprimee).values('etablissement')
        etablissements = set()
    ...
    def _disciplines():
        data = Formation.objects.exclude(
            statut=StatutsFormation.supprimee).values(
    ...
    def _pays():
        data = Formation.objects.exclude(
            statut=StatutsFormation.supprimee).values('etablissement__pays')
        pays = set()
